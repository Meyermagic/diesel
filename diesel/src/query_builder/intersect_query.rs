...
#[derive(Debug)]
pub struct IntersectQuery<L, R> {
    left: L,
    right: R,
}

impl<L, R> IntersectQuery<L, R> {
...
impl<L, R> Query for IntersectQuery<L, R>
...
impl<L, R> CombinableQuery for IntersectQuery<L, R> where IntersectQuery<L, R>: Query {}

impl<L, R, DB> QueryFragment<DB> for IntersectQuery<L, R>
    where DB: Backend,
          L: QueryFragment<DB>,
          R: QueryFragment<DB>
{
...
impl_query_id!(IntersectQuery<L, R>);
